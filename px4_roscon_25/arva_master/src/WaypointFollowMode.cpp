...
void WaypointFollow::getParameters()
{
    _node.declare_parameter("waypoint_tolerance", 0.5);

    _wp_tollerance = _node.get_parameter("waypoint_tolerance").as_double();

    RCLCPP_INFO(_node.get_logger(), "\nParmeters Loaded:\n - Waypoint Tollerance: %f", _wp_tollerance);
}
...
bool WaypointFollow::checkIfReached() const
{
    ...
    if ((_trajectory_waypoints[_current_wp_idx] - current_position).norm() < _wp_tollerance)
    {
        return true;
    }
    return false;
